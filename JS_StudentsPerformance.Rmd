...
In the section 3(Modelling), We approach some algorithms such as Guessing, Regression, Decision Trees, KNN and Random Forest method training each model with trainset and predicting with testset. The models evaluate by accuracy instrument. In the section 4(Results), the result table is shown on which we select the best optimal model. In the section 5(Conclusion), we conclude by presenting summary, potential impact, limitation of the report and future work. 
...
## 3. Modelling
In this section, the process and technique is the following.
...
Evaluation instrument was accuracy. It is more suited with classification data whereas RMSE and R^2 is better suited with continuous data. Therefore, there is limitation to compare classification models and the continuous model with the accuracy measure. 
